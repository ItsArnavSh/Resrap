...
const (
	identifier TokenType = iota //Normal words
	str                         //'...'
	regex                       //[...]
	bracopen                    //(
	bracclose                   //)
...
func (s *scanner) scan() ([]Token, []ScanError) {
	...
	for s.next() != -1 {
		...
		default:
			...
			if buff != "" {
				s.tokens = append(s.tokens, Token{0, word, buff})
			}
		}
	}
...
// 		endNode = s.synG.GetNode(s.generator("end")) //One endnode per block
// 	}
...
// 			wordNode := s.synG.GetNode(s.generator("{" + token.data + "}"))
// 			bufferNode.AddEdgeNext(&s.synG, wordNode, prob(i))
// 			jumpNode := s.synG.GetNode(s.generator("jmp"))
...
// 			leafNode := s.synG.GetNode(s.generator("{" + token.data + "}"))
...
// }
// func (s *scanner) generator(typ string) string {
// 	s.index++
// 	return typ + strconv.Itoa(s.index)
// }
